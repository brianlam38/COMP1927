...
int numOfPossHunter(DracView gameState, Map map, Map railMap, LocationID dest, int *knownLoc,
                    ...
                    LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int hHealth[NUM_HUNTERS], LocationID *path);
...
LocationID predictDracMove(DracView gameState, Map map, Map railMap, LocationID dracSrc, LocationID dracLoc, Round round,
...
LocationID predictHunterMove(DracView gameState, Map map, Map railMap, PlayerID hunter,
                             Round currRound, int hHealth[NUM_HUNTERS],
...
// check if dracula should go to a place which will cause a teleport in next round
int shouldTeleport(Map map, Map railMap, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], Round currRound, LocationID *path);
...
LocationID escapeToSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
                       ...
                       int health, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int hHealth[NUM_HUNTERS],
                       LocationID trail[TRAIL_SIZE], LocationID hideTrail[TRAIL_SIZE], LocationID tmpTrail[TRAIL_SIZE], LocationID tmpHideTrail[TRAIL_SIZE], LocationID *path);
...
// determine whether dracula should stay in a location to leave traps or not
int stayToLeaveTrap(DracView gameState, Map map, Map railMap, LocationID dest, int numPossHunt, Round currRound, int *knownLoc, int knownNumLoc, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int hHealth[NUM_HUNTERS], LocationID trail[TRAIL_SIZE], LocationID *path);
    ...
    if (currRound == 0) {
        ...
        if (currRound > 6) {
            if (isGonnaWin(gameState, fallOffLoc, health)) {
                currLoc = stayInSea(gameState, map, railMap, adjLoc, numLoc,
                                    ...
                                    health, hTrail, hHealth, trail, hideTrail, path);
                submitPlay(gameState, adjLoc, map, railMap, path, currLoc, TRUE,
        ...
        if (isDangerous(map ,railMap, knownLoc, knownNumLoc, distCurrFromSea, distCurrFromHunt,
            ...
            if (idToType(dCurrLoc) == SEA) {
                currLoc = escapeToCity(gameState, map, railMap, adjLoc, numLoc, currRound, knownLoc, knownNumLoc, distCurrFromHunt, health, hTrail, hHealth, trail, hideTrail, tmpTrail, tmpHideTrail, path, area, distAdjust);
            } else {
                int minLength;
                currLoc = escapeToSea(gameState, map, railMap, adjLoc, numLoc, &minLength,
                                  currRound, knownLoc, knownNumLoc, health, hTrail, hHealth, trail, hideTrail, tmpTrail, tmpHideTrail, path);
            }
            submitPlay(gameState, adjLoc, map, railMap, path, currLoc, TRUE,
        ...
        if (dCurrLoc == CASTLE_DRACULA) {
            currLoc = stayOrLeaveCD(gameState, map, railMap, adjLoc, numLoc,
                          currRound, knownLoc, knownNumLoc,
                          health, hTrail, hHealth, trail, hideTrail, tmpTrail, tmpHideTrail, path);
            submitPlay(gameState, adjLoc, map, railMap, path, currLoc, TRUE,
...
//        }
        ...
        if (area != CURR_EMPTY && area != -1) {
            currLoc = escapeToEmptyArea(gameState, map, railMap, adjLoc, numLoc, currRound, knownLoc, knownNumLoc, distCurrFromHunt, health, hTrail, hHealth, trail, hideTrail, path, area, distAdjust);
            submitPlay(gameState, adjLoc, map, railMap, path, currLoc, TRUE,
        ...
        for (i = 0; i < numLoc; i++) {
            ...
            int numPossHunt = numOfPossHunter(gameState, map, railMap, adjLoc[i], knownLoc,
                                              knownNumLoc, currRound, trail, hTrail, hHealth, path);
...
int numOfPossHunter(DracView gameState, Map map, Map railMap, LocationID dest, int *knownLoc,
    ...
    if (knownNumLoc > 0) {
        for (hunter = PLAYER_LORD_GODALMING; hunter < NUM_HUNTERS; hunter++) {
            if (dest == predictHunterMove(gameState, map, railMap, hunter, currRound, hHealth, trail, knownLoc,
...
int shouldTeleport(Map map, Map railMap, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], Round currRound, LocationID *path) {
    PlayerID hunter;
    for (hunter = PLAYER_LORD_GODALMING; hunter < NUM_HUNTERS; hunter++) {
        if (hunterPathLength(map, railMap, hTrail[hunter][0], KLAUSENBURG, hunter, currRound+1, path) <= 3) return 0;
        if (hunterPathLength(map, railMap, hTrail[hunter][0], GALATZ, hunter, currRound+1, path) <= 3) return 0;
    }
    return 1;
}
...
// to predict the moves of dracula in next few round when one of the dracula's location is known
// predict where dracula will go in round round
LocationID predictDracMove(DracView gameState, Map map, Map railMap, LocationID dracSrc, LocationID dracLoc,  Round round, Round currRound, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE],
                           LocationID *path) {
    PlayerID hunter;
    LocationID predictLoc = CASTLE_DRACULA;
    int length;
    int nearestLength;
    int maxLength = -1;
    int numLoc;
    ...
    for (i = 0 ; i < numLoc ; i++) {
        ...
        if (idToType(adjLoc[i]) == idToType(dracLoc)) {
            if (nearestLength > maxLength) {
                maxLength = nearestLength;
                predictLoc = adjLoc[i];
            } else if (nearestLength == maxLength) {
...
LocationID predictHunterMove(DracView gameState, Map map, Map railMap, PlayerID hunter,
    ...
    LocationID huntMove;
    LocationID dracLoc;
    if (knownLoc[knownNumLoc-1] == 0) dracLoc = trail[0];
    else dracLoc = trail[knownLoc[knownNumLoc-1]-1];
    LocationID dracMove = predictDracMove(gameState, map, railMap, trail[knownLoc[knownNumLoc-1]],
                                          dracLoc, currRound - knownLoc[knownNumLoc-1],
                                          currRound, hTrail, path);
    LocationID *predictPath = calloc(NUM_MAP_LOCATIONS, sizeof(LocationID));
    int i;
    
    for (i = 1; i <= knownLoc[knownNumLoc-1]; i++) {
        if (knownLoc[knownNumLoc-1]-i-1 < 0) dracLoc = trail[0];
        else dracLoc = trail[knownLoc[knownNumLoc-1]-i-1];
        dracMove = predictDracMove(gameState, map ,railMap, dracMove, dracLoc, currRound - knownLoc[knownNumLoc-1] + i, currRound, hTrail, path);
    ...
    if (inArray(trail, hunter, TRAIL_SIZE) != -1) {
        int numTraps = 0;
        int numVamps = 0;
        whatsThere(gameState, huntMove, &numTraps, &numVamps);
        if (hHealth[hunter] <= numTraps * 2) return ST_JOSEPH_AND_ST_MARYS;
    }
    free(predictPath);
    return huntMove;
}
...
int shouldGoToCD(Map map, Map railMap, LocationID dCurrLoc, LocationID *adjLoc, int numLoc, Round currRound, LocationID *path, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int distCurrFromHunt, LocationID trail[TRAIL_SIZE], LocationID hideTrail[TRAIL_SIZE]) {
    ...
    if (lengthToCD <= 4 && distCurrFromHunt - 1 > lengthToCD) return 1;
//    for (hunter = PLAYER_LORD_GODALMING; hunter < NUM_HUNTERS; hunter++) {
...
int isDangerous(Map map, Map railMap, int *knownLoc, int knownNumLoc,
    ...
    if (knownNumLoc > 0) {
        if (knownLoc[knownNumLoc-1] < 3 && distFromHunt <= 2) {
...
LocationID escapeToSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    *minLength = NUM_MAP_LOCATIONS;
    LocationID currLoc;
    int shouldTP = shouldTeleport(map, railMap, hTrail, currRound, path);
    ...
    for (x = 0; x < numLoc; x++) {
        ...
        if (adjLoc[x] == CASTLE_DRACULA) possHunt = 3;
        if (stayToLeaveTrap(gameState, map, railMap, adjLoc[x], numPossHunt, currRound, knownLoc, knownNumLoc, hTrail, hHealth, trail, path) == 1) numPossHunt = 0;
        nNextAdj = numOfNextAdj(adjLoc[x], trail, hideTrail, &needHiding);
        if (nNextAdj == 0 && shouldTP == 0) numPossHunt = 2;
        else if (nNextAdj == 0 && shouldTP == 1) length = 0;
...
LocationID escapeToCD(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    *minLength = NUM_MAP_LOCATIONS;
    ...
    LocationID tmpTrail[TRAIL_SIZE];
    char *locAbbrev = malloc(3 * sizeof(char));
    int i, hideAllow, nNextAdj, needHiding, lengthToCD, count = 0;
    ...
    for (i = 0; i < numLoc; i++) {
        ...
        if (possHunt == 0 && nNextAdj == 0) return adjLoc[i];
        else if (nNextAdj == 0) {
            lengthToCD = 1;
            locScore += 50;
        }
        if (stayToLeaveTrap(gameState, map, railMap, adjLoc[i], numPossHunt, currRound, knownLoc, knownNumLoc, hTrail, hHealth, trail, path) == 1) numPossHunt = 0;
...
LocationID escapeToCity(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
                       ...
                       int health, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int hHealth[NUM_HUNTERS],
                       LocationID trail[TRAIL_SIZE], LocationID hideTrail[TRAIL_SIZE], LocationID tmpTrail[TRAIL_SIZE], LocationID tmpHideTrail[TRAIL_SIZE], LocationID *path, int area, int distAdjust) {
    int x, nNextAdj, nCurrNextAdj, needHiding, currNeedHiding, length, minLength = NUM_MAP_LOCATIONS;
    int numHunt, minNumHunt = NUM_HUNTERS;
    int numPossHunt, minNumPossHunt = NUM_HUNTERS;
    int possHunt, minPossHunt = NUM_HUNTERS;
    int locScore, maxScore = -1000;
    ...
    for (x = 0; x < numLoc; x++) {
        ...
        nNextAdj = numOfNextAdj(adjLoc[x], trail, hideTrail, &needHiding);
        if (stayToLeaveTrap(gameState, map, railMap, adjLoc[x], numPossHunt, currRound, knownLoc, knownNumLoc, hTrail, hHealth, trail, path) == 1) numPossHunt = 0;
        if (nNextAdj == 0 && shouldTP == 0) numPossHunt = 1;
        else if (nNextAdj == 0 && shouldTP == 1) length = 0;
        ...
        if (isBestLoc(adjLoc[x], possHunt, &minPossHunt, numPossHunt,
                      ...
                      &minLength, locScore, &maxScore)) {
            currLoc = adjLoc[x];
            nCurrNextAdj = nNextAdj;
            currNeedHiding = needHiding;
            distFromHunt = distFromNearestHunter(map, railMap, adjLoc[x], path, currRound, hTrail);
        }
    }
    //printf("needHiding -= %d, nnext = %d\n", needHiding, nNextAdj);
    if (/*minPossHunt != 0*/(distFromHunt <= 2 || currNeedHiding != 0 || nCurrNextAdj <= 3) && minLength != 0) {
        
        if (area != CURR_EMPTY && area != -1) {
            currLoc = escapeThroughSea(gameState, map, railMap, adjLoc, numLoc, currRound, knownLoc, knownNumLoc, health, hTrail, hHealth, trail, hideTrail, path, area);
        } else {
            currLoc = stayInSea(gameState, map, railMap, adjLoc, numLoc,
                            currRound, knownLoc, knownNumLoc,
                            health, hTrail, hHealth, trail, hideTrail, path);
    ...
    if (area != CURR_EMPTY && area != -1 && minLength != 0) {
        currLoc = escapeToEmptyArea(gameState, map, railMap, adjLoc, numLoc, currRound, knownLoc, knownNumLoc, distCurrFromHunt, health, hTrail, hHealth, trail, hideTrail, path, area, distAdjust);
    }
    return currLoc;
}
...
LocationID escapeToEmptyArea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    for (i = 0; i < numLoc; i++) {
        ...
        if (needHiding != 0) locScore -= 40;
        if (nNextAdj == 0 && shouldTP == 0) locScore -= 200;
        else if (nNextAdj == 0 && shouldTP == 1) locScore += 200;
        if (stayToLeaveTrap(gameState, map, railMap, adjLoc[i], numPossHunt, currRound, knownLoc, knownNumLoc, hTrail, hHealth, trail, path) == 1) numPossHunt = 0;
...
LocationID escapeThroughSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    for (i = 0; i < numLoc; i++) {
        ...
        else if (stayToLeaveTrap(gameState, map, railMap, adjLoc[i], numPossHunt, currRound, knownLoc, knownNumLoc, hTrail, hHealth, trail, path) == 1) numPossHunt = 0;
        distFromHunt = distFromNearestHunter(map, railMap, adjLoc[i], path, currRound, hTrail);
...
int stayToLeaveTrap(DracView gameState, Map map, Map railMap, LocationID dest, int numPossHunt, Round currRound, int *knownLoc, int knownNumLoc, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int hHealth[NUM_HUNTERS], LocationID trail[TRAIL_SIZE], LocationID *path) {
    ...
    if ((numPossHunt > 0) && (currRound % 13 != 0) && (idToType(dest) != SEA)) {
        int numTraps = 0;
        int numVamps = 0;
        whatsThere(gameState, dest, &numTraps, &numVamps);
        // staying in location doesn't affect number of traps
        if (numTraps == 3) return 0;
        
        numTraps++;
        for (hunter = PLAYER_LORD_GODALMING ; hunter <NUM_HUNTERS; hunter++) {
            if (predictHunterMove(gameState, map, railMap, hunter, currRound, hHealth, trail, knownLoc, knownNumLoc, hTrail, path) == dest) {
                if (hHealth[hunter] >= (numTraps * 2)) return 0;
                while (hHealth[hunter] > 0 && numTraps >= 0) {
                    hHealth[hunter] -= 2;
                    numTraps--;
                }
            }
...
int isConcerned(Map map, Map railMap, LocationID dCurrLoc, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE],
    ...
    for (hunter = PLAYER_LORD_GODALMING; hunter < NUM_HUNTERS; hunter++) {
//printf("hunter=%d, from %s, currRound=%d\n", hunter, idToName(hTrail[hunter][0]), currRound);
...
LocationID stayInSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    if (idToType(trail[0]) == SEA) {
        ...
        for (i = 0; i < numLoc; i++) {
            ...
            distFromHunt = distFromNearestHunter(map, railMap, adjLoc[i], path, currRound, hTrail);
            if (adjNumLoc == 0 && shouldTP == 0) {
                distFromHunt = 1;
                count = 0;
            } else if (adjNumLoc == 0 && shouldTP == 1) {
                distFromHunt = 40;
                count = 20;
            }
