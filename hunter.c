...
void lordGodalmingMove(HunterView h) {
  ...
  if (whereIs(h,player) != UNKNOWN_LOCATION) submitID(whereIs(h,player));
  else {registerBestPlay("ST","First Move"); return; }
 //Where are the other players
  int i = 0;

  int numPlaces;
  LocationID *placesToGo = whereCanIgo(h,&numPlaces,1,1,1);
  ...
  if (numPlaces > 1) submitID(placesToGo[1]);
  ...
  if (whereIs(h,PLAYER_DRACULA) <= 70 && whereIs(h,player) >= 0)
  submitID(howToGetTo(whereIs(h,PLAYER_DRACULA),whereIs(h,player),player,giveMeTheRound(h)));
...
//Gives the next step to ge to a certain Location
static LocationID howToGetTo(LocationID dest, LocationID from, int player, Round round) {
